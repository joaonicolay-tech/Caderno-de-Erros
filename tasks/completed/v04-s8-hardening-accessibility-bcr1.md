...
- Final state: `READY_FOR_PILOT`.
- A4 plan: `tasks/plans/v04-s8-hardening-accessibility-bcr1.md`, `COMPLETED`.
- BCR-1: `quality/v04-s8-bcr1-result.md` e JSON bruto, três execuções `PASS`.
- Accessibility: `quality/v04-s8-accessibility-result.md`, com limitações
  explícitas e sem alegação de conformidade WCAG integral.
- Candidate: `quality/v04-s8-candidate-result.md`.
- S5: exit 0, 17 checks, zero findings, conexão read-only.
- S6: `quality/v04-s8-recovery-result.json`, `PASS`, restore isolado e S5 exit 0.
- S7: startup externo/loopback/shutdown sem órfão e backup/validation exit 0.
- Focused regression: 127 passed; final affected subset: 21 passed; BCR tests:
  8 passed.
- Authoritative gate: exit 0, 327 passed in 78.92 s, 88% coverage, all controls
  GREEN, total gate duration 114.3 s after an inconclusive sandbox-network run.
- Deep A8 review: `APPROVED`, no open Blocker, Major or Minor.
- No migration, schema change, S9/pilot, promotion, commit, push, tag or release.
