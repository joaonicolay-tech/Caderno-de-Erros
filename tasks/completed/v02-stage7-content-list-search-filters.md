...
- criada a consulta somente leitura do catálogo no Workspace local, com lista
  `ACTIVE` padrão e seleção explícita de `DRAFT` e `ARCHIVED`;
- criada busca textual simples por enunciado e explicação, sem FTS/FTS5;
- criados filtros de disciplina, assunto e subassunto, validados no Workspace e
  na hierarquia, com filtros ativos, total e estado vazio;
- implementadas paginação limitada, ordenação determinística por
  `updated_at`/UUID e recuperação discreta de página inválida;
- preservados escaping, semântica, foco/teclado, responsividade, detalhes
  protegidos por Workspace e ausência de qualquer escrita na consulta;
- CTs aplicáveis cobertos por `tests/test_question_search.py`; testes focados
  (6) e suíte completa (189) aprovados;
- Ruff, mypy, cobertura (85,47%), `detect-secrets`, `pip-audit`, migrations e
  `git diff --check` aprovados;
- gate autoritativo: GREEN, exit code 0; nenhum P0/P1 aplicável aberto;
- nenhuma migration, capacidade da Etapa 8 ou outra funcionalidade futura foi
  antecipada.
