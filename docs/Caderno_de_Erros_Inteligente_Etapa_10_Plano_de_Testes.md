...
| Próximo marco após aprovação | Início da implementação — V0.1 Fundação Executável |
...
| Tags | Não V0.1; fase pendente antes da V0.2 | Resolver `COR-P1-003` antes de qualquer implementação. |
...
| `CT-003` | Normalizar nomes | `RF-003`–`RF-008`, `RN-004` | Nomes com espaços, caixa e acentos | Cadastrar equivalentes normalizados | Duplicata é rejeitada no mesmo pai; acentos legítimos são preservados | P1 | V0.2 |
| `CT-004` | Impedir hierarquia inválida | `RF-003`–`RF-008`, `RN-003` | Assunto de disciplina A; subassunto informado para B | Salvar vínculo cruzado | Validação rejeita e transação não persiste parcial | P0 | V0.2 |
| `CT-005` | Excluir rascunho da prática | `RF-009`, `RN-013` | Questão sem gabarito, estado rascunho | Buscar, iniciar tentativa e consultar métricas | Não aparece como praticável nem entra em denominadores | P0 | V0.2 |
| `CT-006` | Validar questão ativa | `RF-009`–`RF-014`, `RN-014` | Enunciado e alternativas incompletas | Tentar ativar | Mensagens indicam campos; estado continua rascunho | P1 | V0.2 |
| `CT-007` | Garantir um gabarito | `RF-012`, `RN-016` | Quatro alternativas; zero ou duas corretas | Salvar como ativa | Rejeição; somente uma correta permite ativação | P0 | V0.2 |
| `CT-008` | Versionar conteúdo | `RF-015`–`RF-018`, `RN-018` | Questão ativa sem tentativa | Editar enunciado e salvar | Nova revisão atual; anterior preservada e auditável | P1 | V0.2 |
| `CT-009` | Proteger gabarito usado | `RF-017`, `RN-019` | Questão com tentativa válida | Alterar alternativa correta diretamente | Operação bloqueada ou encaminhada ao fluxo formal V1 | P0 | V0.2 |
| `CT-010` | Arquivar sem apagar histórico | `RF-019`, `RN-020` | Questão com tentativas/revisão pendente | Arquivar e consultar histórico/fila | Histórico permanece; questão sai da prática e pendência fica suspensa | P0 | V0.2 |
| `CT-011` | Cadastro rápido íntegro | `RF-020`, `FL-003` | Taxonomia existente; dados mínimos válidos | Cadastrar pelo fluxo rápido | Questão ativa é criada no pai correto sem campos inconsistentes | P1 | V0.2 |
| `CT-012` | Validar Unicode e limites | `RNF-016`, `RNF-066` | `DAD-004`; fronteiras de tamanho | Salvar mínimo, máximo e máximo+1 | Limites aceitam fronteiras válidas, rejeitam excesso e preservam UTF-8 | P1 | V0.2 |
...
| `CT-075` | Garantir revisão atual única | `MD-DEC-007` | Questão existente | Marcar duas revisões como atuais | Constraint impede duplicidade | P0 | V0.2 |
...
| ID | Objetivo | Rastreabilidade | Pré-condição e entrada | Passos | Resultado esperado | Pri. | Fase |
|---|---|---|---|---|---|---|---|
| `CT-085` | Buscar e filtrar no espaço | `RF-063`, `RNF-002` | `DAD-005`; termos com acento | Buscar, combinar filtros e limpar | Resultados corretos, estáveis e somente do espaço atual | P1 | V0.4 |
| `CT-086` | Paginar sem omissão | `RNF-061` | 10.000 questões; empates de ordenação | Navegar páginas e alterar item | Sem duplicação/omissão; desempate estável | P1 | V0.4 |
| `CT-087` | Salvar filtro V1 | `RF-064` | Filtro composto | Salvar, reabrir, renomear e excluir | Configuração pertence ao espaço e reproduz a consulta | P2 | V0.5-A |
...
| `CT-094` | Preservar entrada após validação | `RNF-007` | Formulário longo com um campo inválido | Submeter | Campos válidos permanecem; erro é claro; nenhuma duplicata criada | P1 | V0.2 |
...
| `CT-124` | Avaliar cadastro rápido | `RNF-006`, `RNF-007` | 5 participantes representativos; roteiro | Criar questão válida e corrigir erro | ≥90% concluem sem ajuda crítica; perda de entrada = zero | P1 | V0.2/V0.4 |
...
| Requisitos Funcionais | `CT-001`–`CT-012`, `CT-013`–`CT-054`, `CT-085`–`CT-094`, `CT-118`–`CT-121`, `CT-127`, `CT-129`–`CT-133`, `CT-135`, `CT-136` |
| Requisitos Não Funcionais | `CT-001`, `CT-002`, `CT-035`–`CT-040`, `CT-073`–`CT-084`, `CT-088`–`CT-136` |
| Regras de Negócio | `CT-001`–`CT-072`, `CT-129`, `CT-135` |
| SDD | `CT-022`, `CT-037`–`CT-040`, `CT-080`, `CT-084`, `CT-101`, `CT-111`, `CT-130`, `CT-132` |
| Modelo de Dados | `CT-073`–`CT-084`, `CT-113`–`CT-120`, `CT-129` |
| Fluxos | `CT-001`, `CT-002`, `CT-011`, `CT-013`–`CT-044`, `CT-085`–`CT-094`, `CT-121`, `CT-129`, `CT-132`, `CT-133`, `CT-136` |
| Roadmap | Gates da seção 8 e `CT-122`–`CT-136` |
...
11. decisões `PT-DEC-001` a `PT-DEC-016`;
...
A etapa foi aprovada integralmente em 30 de agosto de 2026. A errata 1.0.1, registrada em 1º de setembro de 2026, corrige a rastreabilidade aplicável à V0.1, formaliza `CT-129` a `CT-136`, antecipa `CT-127` e ajusta a aplicabilidade de `CT-122`/`CT-123`. Os casos `CT-001` a `CT-136` e as decisões `PT-DEC-001` a `PT-DEC-016` passam a ser considerados congelados e somente poderão ser alterados mediante registro explícito e análise de impacto.
