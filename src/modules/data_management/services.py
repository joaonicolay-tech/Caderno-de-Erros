    ...
    try:
        with closing(_readonly_connection(path, immutable=True)) as database:
            ...
            v03_violations = (
                ...
                "OR r.question_id != a.question_id)) "
                ...
                "OR replaced.attempt_type != a.attempt_type OR replaced.status != 'VOIDED')) "
                "LIMIT 1",
                ...
                "OR a.is_correct = 1 OR (c.state != 'SUPERSEDED' AND a.status != 'VALID'))) "
                "OR (c.origin_kind = 'QUESTION_ACTIVATION' AND c.origin_attempt_id IS NOT NULL) "
                "OR (c.origin_kind = 'MANUAL' AND (a.id IS NULL "
                "OR c.workspace_id != a.workspace_id OR c.question_id != a.question_id "
                "OR a.question_revision_id != qr.id OR a.attempt_type != 'INITIAL' "
                "OR a.is_correct != 1 OR (c.state != 'SUPERSEDED' AND a.status != 'VALID'))) "
                "OR (c.origin_kind = 'ATTEMPT_CORRECTION' AND a.id IS NOT NULL AND ("
                "c.workspace_id != a.workspace_id OR c.question_id != a.question_id "
                "OR a.question_revision_id != qr.id "
                "OR (c.state != 'SUPERSEDED' AND a.status != 'VALID'))) LIMIT 1",
                ...
                "OR r.question_id != a.question_id "
                "OR (r.state = 'PENDING' AND a.status != 'VALID'))) "
                "OR (a.id IS NULL AND NOT (r.sequence_number = 1 AND r.stage_code = 'D1' AND ("
                "(c.origin_kind = 'QUESTION_ACTIVATION' "
                "AND r.transition_code = 'QUESTION_ACTIVATION_D1') "
                "OR (c.origin_kind = 'ATTEMPT_CORRECTION' "
                "AND r.transition_code = 'CORRECTION_RETRY_VOIDED')))) LIMIT 1",
